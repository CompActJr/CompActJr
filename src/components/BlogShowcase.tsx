 ...
 * @description Gerencia a renderização dos artigos do Sanity,
 * com motor de busca client-side e gerador dinâmico de categorias.
...
export default function BlogShowcase({ articlesData }: BlogShowcaseProps) {
    ...
    // Lê todos os artigos, extrai as categorias, remove as duplicadas e adiciona o botão "Todos"
    const dynamicCategories = useMemo(() => {
        if (!articlesData || articlesData.length === 0) return ['Todos'];
        const articleCategories = articlesData.map(article => article.category);
    ...
    return (
        <section className="container mx-auto px-6 max-w-7xl pt-36">
            ...
            {/* Renderização Condicional: Só mostra a busca e a grade se existirem artigos */}
            {articlesData && articlesData.length > 0 ? (
                ...
                <div className="text-center py-24 border border-branco/10 rounded-2xl bg-branco/5">
                    <p className="font-principal text-branco/40 text-lg">Em breve novos artigos e insights serão publicados aqui.</p>
                </div>
            )}
        </section>
    )
